...
func init() {
	...
	flag.BoolVar(&useV2, "v2", false, "Use v0.2 compression algorithm (better compression)")
...
func main() {
	...
	if showVersion {
		fmt.Println("GoZ4X File Compressor v0.2")
		fmt.Println("Go version:", runtime.Version())
		fmt.Println("GOMAXPROCS:", runtime.GOMAXPROCS(0))
		fmt.Println("Using v0.2 algorithm:", useV2)
		os.Exit(0)
	}
	...
	if decompress {
		...
		err = compressFile(inputFile, outputFile, compress.CompressionLevel(level), useV2)
	}
...
func compressFile(inputPath, outputPath string, level compress.CompressionLevel, useV2 bool) error {
	// Open input file
	...
	// Create LZ4 writer
	var lz4Writer io.WriteCloser
	if useV2 {
		lz4Writer = compress.NewWriterWithOptions(outputFile, compress.WriterOptions{
			Level: level,
			UseV2: true,
		})
		fmt.Println("Using v0.2 compression algorithm")
	} else {
